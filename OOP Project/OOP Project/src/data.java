      ...
      List<String> RowDentalClinicData=readFromFile("doctors.txt");
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(DRfilePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                ArrayList<int[]> availableHours = new ArrayList<>();
                String[] hourRanges = fields[7].split("\\$");
                for (String range : hourRanges) {
                    String[] times = range.split(":");
                    int start = Integer.parseInt(times[0]);
                    int end = Integer.parseInt(times[1]);
                    availableHours.add(new int[]{start, end});
                }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(APfilePath))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] fields = line.split(",");
//                String patientUsername = fields[0];
//                String doctorUsername = fields[1];
                String day = fields[0];
                String timeSlot = fields[1];
                 String patient = fields[2] ;
    ...
    public static boolean Store() {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(DRfilePath, true))) {
            for (Doctor doctor : Doctors) {
                ...
                for (int[] range : doctor.getAvailableHours()) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(APfilePath, true))) {
            for (AppointmentReservation appointment : Appointments) {
                StringBuilder line = new StringBuilder();
        ...
        return true; // Successfully cleared and wrote new data
    }
}
